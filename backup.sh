...
for cmd in bash docker borg flock mktemp find stat df sha256sum hostname date grep awk sed tr head; do require_command "$cmd"; done

exec 9>"$LOCK_FILE" || fatal "Lock-Datei kann nicht geöffnet werden: $LOCK_FILE"
flock -n 9 || fatal "Es läuft bereits ein Nextcloud-Backup."

cleanup() {
    local rc=$?
    trap - EXIT INT TERM HUP
    if [[ "$MAINTENANCE_ENABLED" == true ]]; then
        if docker compose -f "$COMPOSE_FILE" up -d "$APP_CONTAINER" >/dev/null 2>&1; then
            ...
            error "Nextcloud konnte beim Cleanup nicht gestartet werden; Maintenance Mode bleibt möglicherweise aktiv."
            rc=1
        fi
        MAINTENANCE_ENABLED=false
    fi
...
for path in "$NC_CONFIG" "$NC_DATA" "$NC_APPDATA" "$NC_EXTERNAL"; do [[ -e "$path" ]] || fatal "Benötigter Pfad fehlt: $path"; done
...
# Conservative preflight: pg_dump can be larger than pg_database_size().
...
info "Aktiviere Nextcloud Maintenance Mode und stoppe danach alle Compose-Dienste außer PostgreSQL."
...
docker compose -f "$COMPOSE_FILE" exec -T -e PGPASSWORD="$DB_PASSWORD" "$DB_CONTAINER" pg_dump -U "$DB_USER" -d "$DB_NAME" --format=plain --no-owner --no-privileges >"$DUMP_FILE" || fatal "pg_dump ist fehlgeschlagen."
chmod 600 "$DUMP_FILE"
